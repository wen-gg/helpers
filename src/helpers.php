...
if (!function_exists('base64_image_format')) {
    ...
    function base64_image_format(string $base64_str)
    {
        ...
        //获取图片后缀
        $suffix_list = [1 => 'gif', 2 => 'jpg', 3 => 'png', 4 => 'swf', 5 => 'psd', 6 => 'bmp', 7 => 'tiff', 8 => 'tiff', 9 => 'jpc', 10 => 'jp2', 11 => 'jpf', 12 => 'jb2', 13 => 'swc', 14 => 'aiff', 15 => 'wbmp', 16 => 'xbm'];
    ...
    function imageroundcorner($image, int $w, int $h, int $r)
    {
        ...
        for ($x = 0; $x < $w; $x++) {
            for ($y = 0; $y < $h; $y++) {
                ...
                $color_info = imagecolorsforindex($image, $rgb_color); // 获取原图颜色信息
                $target_color = imagecolorallocatealpha($img, $color_info['red'], $color_info['green'], $color_info['blue'], $color_info['alpha']);

                if (($x >= $r && $x <= ($w - $r)) || ($y >= $r && $y <= ($h - $r))) {
                    //不在四角的范围内,直接画
                    imagesetpixel($img, $x, $y, $target_color);
                } else {
                    //在四角的范围内选择画
                    //上左
                    $y_x = $r; //圆心X坐标
